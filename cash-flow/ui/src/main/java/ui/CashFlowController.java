...
public class CashFlowController {
  ...
  @FXML TextField nameAccount; 
  @FXML TextField setAmount;
  @FXML TextArea accounts;
  @FXML Button createAccount;
  @FXML Button detailsAndTransfers;
  @FXML Text accountCreated; 
   ...
   * It checks that no fields are left empty. If no fields are empty, 
   * it creates an account and is added to the user where the user has
   * access, either on Remote or Locally (Rest API).
   */
  @FXML
  ...
  /**
   * Checks if the number typed in is valid by being not null and if so, that the number typed in is
  ...
  /**
   * Method for updating the accountview after changes. Based on the accounts type, 
   * it gets its label from what kind of account it is. Format numbers in a readble way.
   */
  ...
  /**
   * When the choicebox is pressed and an item is choosen, it shows the restrictions for the choosen
   * account based on its type. Message is display with the restrictions.
   */
  @FXML
